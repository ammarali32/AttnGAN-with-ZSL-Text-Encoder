## ref https://github.com/taoxugit/AttnGAN/blob/master/code/model.py
...
from layers.GlobalAttention import GlobalAttentionGeneral as ATT_NET
import os
...
from models.modelFromZSL import _netG, DEncoder

# ############## Text2Image Encoder-Decoder #######
class RNN_ENCODER(nn.Module):
    def __init__(self, vocab_size, ninput=300, drop_prob=0.5,
    ...
    def define_module(self):
        #This is the Magic Trick: replace the Text_encoding using nn.Embedding by the encoder used in ZSL_GAN
    ...
    def init_weights(self):
        ...
        if os.path.isfile(configs.checkpoint_model_ZSL):
            self.encoder_netG.load_state_dict(torch.load(configs.checkpoint_model_ZSL))
...
class CNN_ENCODER_DEMO(nn.Module):
    """
    This is DEMO example just to make the code work, It is better to unify the Data between the ZSLGAN and ATTNGAN to make everything work 
    properly. The current solution elimeinate the ranked loss because of the data issue.
    """
    def __init__(self, in_dim, nef):
        super(CNN_ENCODER_DEMO, self).__init__()
        self.nef = nef
        self.emb_cnn_code = nn.Linear(in_dim, self.nef)
        self.conv1x1 = conv1x1(configs.VGG_FEATURES_SIZE, self.nef)

    def forward(self, x):
        
        emb_cnn = self.emb_cnn_code(x)
        x = x.view(-1, configs.VGG_FEATURES_SIZE, configs.IMG_SIZE, configs.IMG_SIZE)
        x = nn.Upsample(size=(configs.INCEPTION_V3_OUTPUT_SIZE, configs.INCEPTION_V3_OUTPUT_SIZE), mode='bilinear')(x)
        #print(x.shape, "SHAPE")
        features = self.conv1x1(x)
        return features, emb_cnn
...
#  def __init__(self, in_dim, nef):
#         super(CNN_ENCODER_SMALL, self).__init__()
#         self.nef = nef
#         self.emb_cnn_code = nn.Linear(in_dim, self.nef)
#         self.conv1x1 = conv1x1(configs.VGG_FEATURES_SIZE, self.nef)
...
class D_NET64(nn.Module):
    ...
    def forward(self, x_var):
        x = x_var.view(-1, 3, 16, 16)
        x = nn.Upsample(size=(64, 64), mode='bilinear')(x)
        x_code4 = self.img_code_s16(x)  # 4 x 4 x 8df
        return x_code4
