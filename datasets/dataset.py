## ref to the repo author https://github.com/EthanZhu90/ZSL_GAN/blob/master/dataset.py
...
class TextDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.type == "train":
            text = np.array([self.dataset.train_text_feature[i, :] for i in labels])
        else:
            text = np.array([self.dataset.test_text_feature[i, :] for i in labels])
        text = Variable(torch.from_numpy(text.astype('float32'))).to(configs.device)
        img = Variable(torch.from_numpy(feat_data)).to(configs.device)
        y_true = Variable(torch.from_numpy(labels.astype('int'))).to(configs.device)
        return img, text, y_true
